...
class Author(UserMixin, db.Model):
    ...
    subscribers = db.relationship(
        "Subscriber", secondary="subscriber_author", backref="authors"
    )
...
class Tag(db.Model):
    ...
    articles = db.relationship(
        "Article", secondary="tag_article", backref="tags"
    )
    subscribers = db.relationship(
        "Subscriber", secondary="subscriber_tag", backref="tags"
    )
...
class Article(db.Model):
    ...
    updated_on = db.Column(
        db.DateTime, default=datetime.utcnow, onupdate=db.func.now()
    )
...
class Asera(db.Model):
    __table_args__ = (
        db.PrimaryKeyConstraint(
            "article_id", "section_id", "region_id", "age_id", "ethnos_id"
        ),
    )
