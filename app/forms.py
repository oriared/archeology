...
from wtforms import ValidationError, StringField, PasswordField, BooleanField, SubmitField, TextAreaField, SelectField, SelectMultipleField
...
class AddArticleForm(FlaskForm):
    ...
    region = SelectMultipleField('Регионы (современные):', coerce=int, validators=[
        DataRequired(message='Необходимо выбрать минимум один регион')])
    ...
    def validate_ethnos(self, ethnos):
        if ethnos.data == -1 and not self.new_ethnos.data:
            raise ValidationError('Выберите этнос из списка или добавьте новый')
