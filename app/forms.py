...
class AddArticleForm(FlaskForm):
    ...
    summary = TextAreaField(
        "Краткое описание, тизер (до 500 символов)",
        validators=[Length(max=500, message="Слишком длинное описание")],
    )
    ...
    new_ethnos = StringField(
        "Добавить новый этнос:",
        validators=[Length(max=30, message="Название этноса слишком длинное")],
    )
    ...
    def validate_ethnos(self, ethnos):
        if ethnos.data == -1 and not self.new_ethnos.data:
            raise ValidationError(
                "Выберите этнос из списка или добавьте новый"
            )
